...
/*    for (var i = 0; i < document.querySelectorAll(".randomPizzaContainer").length; i++) {
      ...
      console.log('changePizzaSizes', document.querySelectorAll(".randomPizzaContainer")[i].style.width );
    };
    */
// 1 st modification
  var randomPizzaObject = document.getElementsByClassName('randomPizzaContainer');
  var length = randomPizzaObject.length;
  var i = 0;
  var dx = determineDx(randomPizzaObject[1], size);
  var newwidth = (randomPizzaObject[i] + dx) + 'px';//console.log('changePizzaSizes', randomPizzaObject);

    for (var i = 0; i < length; i++) {
      randomPizzaObject[i].style.width = newwidth;
    };
}
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var rows = screen.height/70; //2nd modification: visible rows approx. using about 70 px as pizza size
  var visible = cols*rows;
  var elem = ''; //3rd modification: moved here from the for loop below.
  var movingPizzas = document.getElementById('movingPizzas1'); //4th modification: moved from the for loop below.
