...
bioj.bio.display = function() {
    ...
    var formattedcontactGeneric = (HTMLcontactGeneric.replace("%contact%", "Get in touch,")).replace("%data%", "ways:");
...
bioj.work.display = function() {
    ...
    if (job >= 0) {
        ...
        for (var joball in bioj.work.jobs) {
...
bioj.education.display = function() {
    ...
    if (j >= 0) {
        ...
        for (var school in bioj.education.schools) {
            var formattedschoolName = HTMLschoolName.replace("%data%", bioj.education.schools[school].name).concat(HTMLschoolDegree.replace("%data%", bioj.education.schools[school].degree));
    ...
    for (var course in bioj.education.onlineCourses) {
...
bioj.projects.display = function() {
    if (bioj.projects.project.length > 0) {
        ...
        //proceed if there are own projects
        for (var project_i in bioj.projects.project) {
